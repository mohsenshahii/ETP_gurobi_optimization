...
# 2. **Encouraging Gaps Between Conflicting Exams**
gap_bonus = 0
for e1 in exams.keys():
    for e2 in exams.keys():
        if conflicts[e1][e2] > 0 and e1 < e2:
            for d in range(3, 6):  # Reward conflicts scheduled 3+ slots apart
                for t in range(num_slots - d):
                    w = model.addVar(vtype=GRB.BINARY, name=f"w_{e1}_{e2}_{t}_{d}")
                    model.addConstr(w <= x[e1, t])
                    model.addConstr(w <= x[e2, t + d])
                    model.addConstr(w >= x[e1, t] + x[e2, t + d] - 1)
                    gap_bonus += d * w  # Reward longer gaps

# Updated Objective Function (Minimizing penalty while considering fairness)
lambda2 = 0.5  # Weight factors for fairness constraints
model.setObjective(penalty - lambda2 * gap_bonus, GRB.MINIMIZE)
# Solve model
# model.setParam("TimeLimit", 600)  # Remove time limit
model.optimize()
