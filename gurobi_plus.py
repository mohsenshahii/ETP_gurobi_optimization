...
# New Constraint: Limit the number of exams per student per day
max_exams_per_day = 2
for sid, student_exams in students.items():  # Rename `exams` to `student_exams`
    for t in range(num_slots):
        model.addConstr(sum(x[e, t] for e in student_exams) <= max_exams_per_day)
